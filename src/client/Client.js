...
class Client extends DiscordClient {
    ...
    loadEvents(dir) {
        try {
            const events = readdirRecursive(dir)
                .filter(
    ...
    loadCommands(dir) {
        ...
        try {
            const commands = readdirRecursive(dir)
                .filter(
                    (f) => f.endsWith('.js') && !basename(f).startsWith('_'),
                )
                .map((f) => ({
                    props: require(normalize(f)).props,
                    path: normalize(f),
                }));
            ...
            for (const { props, path } of commands) {
                if (!props)
                    return console.log(
                        `Unable to load a command; ID: ${props.id}. No props given`,
                    );
                if (!props.commands || !Array.isArray(props.commands))
                    return console.log(
                        `Unable to load a command; ID: ${props.id}. No valid command array given`,
                    );
                for (const command of props.commands) {
                    if (this.commands.get(command))
                        return console.log(
                            `Unable to load command; ID: ${props.id}. Command word ${command} is taken`,
                        );
                    this.commands.set(command, require(path));
                    console.log(
                        `Loaded command word ${command} for ${props.id}`,
                    );
                }
            }
        } catch (e) {
